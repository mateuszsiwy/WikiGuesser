            ...
            function GamePage({ username, connection }) {
                ...
                useEffect(() => {
                    ...
                    if (connection) {
                        ...
                        if (lobbyId) {
                            connection.on("GameStarted", (receivedLobbyId, articleDTO) => {
                                console.log("Game started event received", receivedLobbyId, articleDTO);
                                if (receivedLobbyId === lobbyId) {
                                    console.log("halo kurwa");
                                    setArticle(articleDTO.articleName);
                                    setCityDesc(articleDTO.summary);
                                    const lat = typeof articleDTO.location.latitude === 'string'
                                        ? parseFloat(articleDTO.location.latitude)
                                        : articleDTO.location.latitude;
                                    const lng = typeof articleDTO.location.longitude === 'string'
                                        ? parseFloat(articleDTO.location.longitude)
                                        : articleDTO.location.longitude;
                ...
                function onSubmit() {
                    ...
                    if (lobbyId && connection) {
                        ...
                        if (currentPlayer) {
                            ...
                            console.log("Sending player scores:", playerScores, "Round:", round);
                            
                            if (round < 5) {
                                setTimeout(() => {
                                    connection.invoke("NextRound", lobbyId, playerScores, round + 1)
                                        .catch(err => console.error("Error sending score:", err));
                                }, 5000);
                            } else {
                                setTimeout(() => {
                                    connection.invoke("EndGame", lobbyId)
